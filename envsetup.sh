...
function prepare_dl
{
	...
	[ -e $T/dl/.finished ] && return 0

	echo -n "Preparing the packages >>>"
	...
	do
		...
		for pkg in `cat $T/build/package_split_list`
		do
			[ -e $pkg.00 ] && cat $pkg.* > $pkg && rm $pkg.*
			echo -n ">"
		done
	...
	local mfs="$T/.repo/manifests/lindenis-v536.xml"
        local needupdate=$(grep "updated" $mfs)
        if [ "$needupdate" = "" ]; then
                sed -i '/dl[0-9]/{s/</<!-- /g};/dl[0-9]/{s/\/>/-->/g};$a<!-- updated -->' $mfs
        fi
...
function prepare_brandy_toolchain
{
	local T=$(gettop)
	local tdir="$T/lichee/brandy/toolchain"

	[ -e $tdir/.finished ] && return 0

	pushd $tdir > /dev/null

	for pkg in `cat $T/build/brtoolchain_split_list`
	do
		[ -e $pkg.00 ] && cat $pkg.* > $pkg
	done
